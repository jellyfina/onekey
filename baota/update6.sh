...
if [ ! -d /www/server/panel/BTPanel ];then
	...
	echo "5.9平滑升级到6.0的命令：curl http://download.yu.al/install/update_to_6.sh|bash"
	exit 0;
fi
...
if [ "${publicFileMd5}" != "${md5check}"  ]; then
	wget -O Tpublic.sh http://download.yu.al/install/public.sh -T 20;
...
setup_path=/www
version=$(curl -Ss --connect-timeout 5 -m 2 https://auth.yu.al/api/panel/get_version)
if [ "$version" = '' ];then
	version='7.7.0'
fi


wget -T 5 -O /tmp/panel.tar.gz $downloads_Url/jellyfina/liebiao/main/panel.tar.gz
dsize=$(du -b /tmp/panel.tar.gz|awk '{print $1}')
if [ $dsize -lt 10240 ];then
	echo "获取更新包失败！"
	exit;
fi
tar zxvf -C /tmp/panel.tar.gz -d $setup_path/server/ > /dev/null
...
wget https://ghproxy.com/https://raw.githubusercontent.com/jellyfina/liebiao/main/plugin.json -T 10
mv plugin.json /www/server/panel/data
